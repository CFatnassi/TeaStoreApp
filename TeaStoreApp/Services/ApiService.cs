...
namespace TeaStoreApp.Services
{
	...
	{
		public static async Task<bool> Register(string name, string email, string phone, string password)
		{
		...
		public static async Task<bool> UplaodUserImage(byte[] imageArray)
		{
		...
		public static async Task<bool> UpdateCartQuantity(int productId, string action)
		{
			...
			httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", Preferences.Get("accesstoken", string.Empty));
			var response = await httpClient.GetAsync(AppSettings.ApiUrl + "api/shoppingcartitems?productId="+productId+"&action="+action);
			if (!response.IsSuccessStatusCode) return false;
			return true;
		}
